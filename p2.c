...
for (i=0;i<10;i++)
{
/* unsigned int sleep(unsigned int seconds): provoca una espera en segundos, en
función del
valor del parámetro */
sleep(1);
time_msg.mtext[0]++;/* int msgsnd(int msg, struct msgbuf * buf, int size, int flag): envía
el mensaje almacenado en
el segundo campo de buf, de tamaño size, a la cola con identificador msg. El cuarto
parámetro
